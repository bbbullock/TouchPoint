...
#
# Major v2.3.0 Change:
#   - Normal exports are restricted to batches dated before the current month.
...
#       - Status = Reconciled
#       - Export Status = No
#       - DepositDate is before the current month, unless Clear Export Flag
#         testing is enabled
#
...
#       - Any selected batch is from a month that is still open, unless Clear
#         Export Flag testing is enabled
...
def get_recent_batches():
    ...
    SELECT TOP 250
        ...
        CASE
            WHEN bh.DepositDate < DATEADD(month, DATEDIFF(month, 0, GETDATE()), 0)
                THEN 1
            ELSE 0
        END AS IsPriorMonth,
...
def validate_selected_batches(cleanlist):
    ...
    for r in summary_rows:
        ...
        deposit_date = row_str(r, ["DepositDate"])
        is_prior_month = row_str(r, ["IsPriorMonth"]) == "1"
        found[bid] = True
        ...
        if not is_prior_month:
            not_prior_month.append("{0} ({1})".format(bid, deposit_date if deposit_date else "blank date"))
    ...
    if not_prior_month and not ENABLE_CLEAR_EXPORT_FLAG:
        errors.append("These selected batches are from a month that is still open and cannot be exported until after the month closes unless the Clear Export Flag testing feature is enabled: " + ", ".join(not_prior_month))
...
# -----------------------------------------------------------------------------
    ...
    for b in batches:
        ...
        is_prior_month = row_str(b, ["IsPriorMonth"]) == "1"
        ...
        elif not is_prior_month and not ENABLE_CLEAR_EXPORT_FLAG:
            eligibility_text = "Month Still Open"
        elif is_exported:
        ...
        elif not is_reconciled or (not is_prior_month and not ENABLE_CLEAR_EXPORT_FLAG):
        ...
        if is_paper:
            ...
            checkbox_disabled = True
            checkbox_title = "Batch can be exported after the month closes"
        ...
        '''.format(
            ...
            "1" if (not is_paper and is_reconciled and not is_exported and (is_prior_month or ENABLE_CLEAR_EXPORT_FLAG)) else "0",
                ...
                <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap">
                    <div>
                        ...
                        <div class="muted" style="margin-top:6px">
                            Version {1} generates the final QBO CSV directly from this tool.
                            Export is allowed only for prior-month batches with Status = <strong>Reconciled</strong>, Exported = <strong>No</strong>, and Batch ID {5} or higher.
                        </div>
                        ...
                        <div class="v2-note">
                            ...
                            Batches from a month that is still open are blocked unless the Clear Export Flag testing feature is enabled.
                            Exported batches can only be selected for clearing when the Clear Export Flag testing feature is enabled.
            ...
            <div class="qbo-card">
                ...
                <div class="setup-row">
                    ...
                    <div class="setup-help">
                        ...
                        It also allows batches from a month that is still open to be exported for testing.
                        When disabled, the button is hidden, already-exported batches cannot be selected, open-month batches cannot be exported, and the server-side clear action is blocked.
                    </div>
                </div>
